...
### 3. **Latest from the Arena** (`components/latest-from-arena.tsx`)
...
### 4. **Featured Match** (`components/featured-match.tsx`)
...
### 5. **Upcoming Highlights** (`components/highlights-premium.tsx`)
...
### 8. **WhatsApp Community** (`components/whatsapp-community.tsx`)
...
1. The current header logo is a simple orange "R" box in `components/header.tsx`
...
In `components/hero-enhanced.tsx`, after the glow effects, add:
```tsx
<motion.img 
  src="/hero-image.png" 
  alt="Stadium" 
