...
- Slogan preserved: Where fans come alive.
...
- Added a social story grid that works for Instagram/X/WhatsApp creative reuse.
- Added a system section explaining why the site stays alive.
- Added a cleaner connect section for Instagram, X, WhatsApp, and email.
- Kept responsive mobile sticky CTA.

## Data/automation check
- `/api/sync/matches` remains scheduled every 30 minutes.
- `/api/admin/auto-curate` remains scheduled every 30 minutes.
- `/api/sync/instagram` remains scheduled.
- `/api/sync/x` remains scheduled for environments with X credentials.
