    ...
    <div className={`group/logo relative grid place-items-center overflow-hidden rounded-[1.35rem] border border-border bg-background/60 p-2.5 shadow-soft-glow ${className}`}>
      ...
      {usableFlagSrc ? (
        <img
          src={flagSrc || ''}
          alt={alt}
          className={`relative h-full w-full rounded-[0.9rem] object-cover drop-shadow-[0_6px_14px_rgba(0,0,0,0.35)] transition-transform duration-300 ease-out group-hover/logo:scale-[1.08] ${imageClassName}`}
          loading="lazy"
          decoding="async"
        ...
        <div className="grid h-full w-full place-items-center rounded-[1rem] bg-primary/10 text-center font-display text-xl uppercase leading-none text-primary sm:text-2xl">
          {initials}
        </div>
      )}
    </div>
