...
function MatchTeam({ match, side, size = 'regular' }: { match: ArenaMatch; side: 'home' | 'away'; size?: 'regular' | 'hero' }) {
  ...
  const logoSize = size === 'hero' ? 'h-20 w-20 sm:h-24 sm:w-24 lg:h-28 lg:w-28' : 'h-20 w-20 sm:h-24 sm:w-24'
...
function MatchScoreBlock({ match, large = false }: { match: ArenaMatch; large?: boolean }) {
  return (
    <div className="flex flex-col items-center text-center">
      <div className={`grid place-items-center rounded-full border border-primary/40 bg-primary/10 shadow-soft-glow ${large ? 'min-h-24 min-w-24 sm:min-h-28 sm:min-w-28' : 'min-h-20 min-w-20'}`}>
        <span className={`font-display uppercase leading-none text-primary ${large ? 'text-4xl sm:text-5xl' : 'text-3xl sm:text-4xl'}`}>{scoreText(match)}</span>
      </div>
      {match.winner === 'draw' ? (
      ...
      {match ? (
        ...
        >
          ...
          >
            ...
            <div className="relative mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-[1.6rem] border border-border bg-background/55 p-4 sm:gap-5 sm:p-5">
...
function MatchPoster({ match, onOpen }: { match?: ArenaMatch; onOpen: (match: ArenaMatch) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-4">
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-primary">
          Open story <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" />
        </span>
      </div>
    </a>
  )
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.08 } } }} className="grid items-center gap-7 lg:grid-cols-[0.92fr_1.08fr] lg:gap-10">
          <div>
            ...
            <motion.h1 variants={reveal} className="mt-5 max-w-2xl font-display text-[clamp(2.8rem,7.2vw,5.7rem)] uppercase leading-[0.94] text-foreground text-balance">
              {siteConfig.hero.headline}
            </motion.h1>
            <motion.p variants={reveal} className="mt-5 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">
              {siteConfig.hero.subheadline} <span className="font-black text-foreground">{siteConfig.brand.tagline}</span>
            </motion.p>
            ...
            <motion.div variants={reveal} className="mt-7 flex flex-col gap-3 sm:flex-row">
              ...
              <a href="#live" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-surface/70 px-6 py-3.5 text-sm font-black uppercase tracking-[0.12em] text-foreground backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-primary/50">
                <Radio className="h-4 w-4" /> {siteConfig.hero.secondaryCta}
              </a>
            </motion.div>

            <motion.div variants={reveal} className="mt-7 grid grid-cols-3 gap-3">
              {[
                { label: 'Live', value: liveCount, icon: Wifi },
                { label: 'Results', value: finalCount, icon: Trophy },
                { label: 'Next', value: upcomingCount, icon: CalendarDays },
              ].map((item) => (
...
function LiveSection({ data, onOpenMatch }: { data: ReturnType<typeof usePublicHome>['data']; onOpenMatch: (match: ArenaMatch) => void }) {
  ...
  return (
    <section id="live" className="relative overflow-hidden bg-surface py-14 sm:py-20 lg:py-20">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          ...
          body="Tap any card for the full match view. Winners, draws, live games, and upcoming fixtures stay readable across desktop and mobile."
        />
  ...
  return (
    <section id="updates" className="relative bg-background py-14 sm:py-20 lg:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          eyebrow="Stories"
          title="Built for every feed. Clean on the site."
          body="Post one image across Instagram, X, and WhatsApp. Roar Arena turns it into a proper story grid with category, context, and matchday energy."
        />
...
function EngineSection() {
  const cards = [
    { icon: RefreshCw, title: 'Every 30 minutes', body: 'Match sync checks scores and fixtures on a schedule so the board does not go stale after a big result.' },
    { icon: Database, title: 'Cached in Supabase', body: 'The site reads saved data, not the sports API on every visit. That protects the free API limit.' },
    { icon: ShieldCheck, title: 'Manual safety layer', body: 'Admin can pin, hide, feature, or correct stories when the fan moment needs taste, not just automation.' },
  ]

  return (
    <section id="engine" className="relative overflow-hidden bg-surface py-14 sm:py-20 lg:py-20">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/70 to-transparent" />
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-primary">Self-sustaining system</p>
            <h2 className="mt-3 font-display text-[clamp(2.2rem,5.8vw,4.8rem)] uppercase leading-[0.98] text-foreground text-balance">Not a static landing page.</h2>
            <p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground">
              The site works like a matchday control room: live data in, social posts in, hero story out. Roar Arena stays alive without rebuilding the homepage every time the sports world changes.
            </p>
            <div className="mt-7 flex flex-wrap gap-2">
              {siteConfig.hero.badges.map((badge) => (
                <span key={badge} className="rounded-full border border-border bg-card px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-muted-foreground">{badge}</span>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
            {cards.map((card) => (
              <div key={card.title} className="rounded-[1.7rem] border border-border bg-card p-6 shadow-soft-glow">
                <div className="grid h-12 w-12 place-items-center rounded-2xl bg-primary/10 text-primary">
                  <card.icon className="h-6 w-6" />
                </div>
                <h3 className="mt-5 font-display text-2xl uppercase leading-none text-foreground sm:text-3xl">{card.title}</h3>
...
function SportsSection() {
  return (
    <section id="sports" className="bg-background py-14 sm:py-20 lg:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          eyebrow="Coverage"
          title="Every sport gets its own roar."
          body="Football is the live focus now, but the brand system is ready for basketball, cricket, racing, fight nights, and major global events."
        />
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {siteConfig.sports.map((sport) => (
            <div key={sport.name} className="group rounded-[1.5rem] border border-border bg-card p-5 text-center transition duration-300 hover:-translate-y-1 hover:border-primary/45 hover:shadow-soft-glow">
              <AssetLogo src={sport.logo} alt={`${sport.name} logo`} className="mx-auto h-20 w-20 bg-black/25 p-3" />
              <h3 className="mt-4 text-sm font-black uppercase text-foreground">{sport.name}</h3>
              <p className="mt-1 text-xs font-bold text-primary">{sport.label}</p>
            </div>
          ))}
        </div>
      </div>
...
function ConnectSection() {
  const links = [
    { label: 'Instagram', body: 'Main creative feed', href: siteConfig.links.instagram, icon: Flame },
    { label: 'X', body: 'Fast matchday takes', href: siteConfig.links.x, icon: Zap },
    { label: 'WhatsApp', body: 'Channel alerts', href: siteConfig.links.whatsappChannel, icon: MessageCircle },
    { label: 'Email', body: siteConfig.links.contactEmail, href: siteConfig.links.contact, icon: Mail },
  ]

  return (
    <section id="connect" className="relative overflow-hidden bg-surface py-14 sm:py-20 lg:py-20">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-[2rem] border border-border bg-card p-6 shadow-soft-glow sm:p-8 lg:p-10">
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-primary">Connect</p>
              <h2 className="mt-3 font-display text-[clamp(2.2rem,5.8vw,4.8rem)] uppercase leading-[0.98] text-foreground">Enter the arena.</h2>
              <p className="mt-5 text-base leading-7 text-muted-foreground">Follow the posts, join the channel, and reach out for screenings, collabs, tournaments, or fan events.</p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {links.map((link) => (
                <a key={link.label} href={link.href} target={link.label === 'Email' ? undefined : '_blank'} rel={link.label === 'Email' ? undefined : 'noopener noreferrer'} className="group rounded-[1.5rem] border border-border bg-background/70 p-5 transition hover:-translate-y-1 hover:border-primary/45">
  ...
  return (
    <section className="relative overflow-hidden bg-background py-14 sm:py-20 lg:py-20">
      ...
      <div className="relative mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <h2 className="mt-6 font-display text-[clamp(2.6rem,8vw,5.8rem)] uppercase leading-[0.95] text-foreground">Where fans come alive.</h2>
        <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-muted-foreground">
          Scores bring the result. Roar Arena brings the feeling around it.
        </p>
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <a href={siteConfig.links.whatsappChannel} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3.5 text-sm font-black uppercase tracking-[0.12em] text-primary-foreground shadow-soft-glow transition hover:-translate-y-0.5 hover:shadow-glow">
            <Bell className="h-4 w-4" /> Join the channel
          </a>
          <a href={siteConfig.links.x} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-surface px-6 py-3.5 text-sm font-black uppercase tracking-[0.12em] text-foreground transition hover:-translate-y-0.5 hover:border-primary/50">
            <CheckCircle2 className="h-4 w-4" /> Follow the feed
          </a>
        </div>
      </div>
    </section>
