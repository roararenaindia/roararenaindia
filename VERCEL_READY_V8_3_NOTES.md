...
## Fixed
- Added `siteConfig.futureEvents` so `components/future-events.tsx` type-checks.
- Kept all icon usage on the local `components/icon-set.tsx` system instead of direct `lucide-react` exports.
...
## Automation retained
- `/api/sync/matches` every 30 minutes.
